...
public class Java_LLParserAnalysisRec {
    ...
    private static void get_table() {
        for (String nonEndChar : nonEndChars) {
            ...
            for (String rightItem : rightItems) {
                ...
                HashSet<String> rightFirst = get_rightItems_first(rightItem);
     ...
     * 使用递归下降的语法分析方法
     */
    private static Map<String,Grammar> mapMethod;
    private static int inputIndex;
    private static class Grammar{
        String nonTerminal;
        String[] lefts;

        public Grammar(String nonTerminal, String[] lefts) {
            this.nonTerminal = nonTerminal;
            this.lefts = lefts;
        }

        public void method(){
            for(String left:this.lefts){
                int recordInput = inputIndex;
                String[] Chars = left.split(" ");
                for(String Char:Chars){
                    //xi是非终结符，调用Xi()
                    if(nonEndChars.contains(Char)){
                        mapMethod.get(Char).method();
                    }
                    //xi等于输入符号a, 读入下一个符号
                    else if(Char.equals(prog.get(inputIndex).getKey())){
                        inputIndex++;
                    }
                    //发生错误
                    else{
                ...
                //回溯
                if(inputIndex == prog.size()){
                    break;
                }
                else{
                    inputIndex = recordInput;
                }
            }
        };
    }
    ...
    private static void analysis_rec(){
        Grammar program = new Grammar("program",new String[]{"compoundstmt"});
        Grammar stmt = new Grammar("stmt", new String[]{"compooundstmt","ifstmt","whilestmt","assgstmt"});
        Grammar compoundstmt = new Grammar("compoundstmt",new String[]{"{ stmts }"});
        Grammar stmts = new Grammar("stmts", new String[]{"stmt stmts","E"});
        Grammar ifstmt = new Grammar("ifstmt",new String[]{"if ( boolexpr ) then stmt else stmt"});
        Grammar whilestmt = new Grammar("whilestmt", new String[]{"while ( boolexpr ) stmt"});
    }
